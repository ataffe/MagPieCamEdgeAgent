...
class STrack(BaseTrack):
    ...
    def __init__(self, tlwh, score, category):
        ...
        # wait activate
        # x, y, w, h
        self._tlwh = np.squeeze(np.asarray(tlwh, dtype=np.float64))
    ...
    def __repr__(self):
        return f'STrack(track_id={self.track_id}, state={self.state}, category={self.category},' \
    f' tlwh={self.tlwh}, score={self.score})'
...
class BYTETracker(object):
    ...
    def update(self, scores, bboxes, categories, img_wh):
        self.frame_id += 1
        activated_starcks = []
        refind_stracks = []
        lost_stracks = []
        ...
        categories_second = categories[inds_second]
        scores_second = scores[inds_second]

        dets = bboxes[remain_inds]
        scores_keep = scores[remain_inds]
        categories_keep = categories[remain_inds]

        if len(dets) > 0:
            '''Detections'''
            detections = [STrack(STrack.tlbr_to_tlwh(box), score, cat) for
                          (box, score, cat) in zip(dets, scores_keep, categories_keep)]
        else:
            detections = []
        ...
        if len(dets_second) > 0:
            '''Detections'''
            detections_second = [STrack(STrack.tlbr_to_tlwh(box), score, cat) for
                          (box, score, cat) in zip(dets_second, scores_second, categories_second)]
        else:
            detections_second = []
